...
let handler = async (m, { conn, usedPrefix, text, command }) => {
  try {
    ...
    if (!global.bannerUrls[conn.user.jid]) {
      global.bannerUrls[conn.user.jid] = 'https://qu.ax/bBoMq.jpg'; // URL inicial de la imagen del menú
    }
    ...
    if (command === 'menu' || command === 'help' || command === 'menú') {
      ...
      let perfil = await conn.profilePictureUrl(who, 'image').catch(_ => 'https://qu.ax/bBoMq.jpg');
